...
class LogisticClassifier(object):
    ...
    def feed_forward(self, x):
        ...
        sigmoid_vector = np.vectorize(sigmoid)  # or use a different name if you want to keep the original f

        

        result = None 
        result = sigmoid_vector(x.dot(self.w))

        #print(x.shape, self.w.shape, result.shape)
    ...
    def compute_loss(self, y, y_hat):
        ...
        loss = 0
        loss = -1.0* np.mean(np.multiply(y, np.log(y_hat) + np.multiply((1.0-y), np.log(1.0-y_hat))))
        return loss
    ...
    def update_weight_momentum(self, grad, learning_rate, momentum, momentum_rate):
        ...
        self.w = self.w - learning_rate * grad - momentum_rate * momentum
...
def normalize_per_pixel(train_x, test_x):
    ...
    # test_x = ...
    def normalize(x):
        mean = x.mean(axis=0) # calculates the mean of the array x
        mean = np.repeat(mean[np.newaxis,:, :], x.shape[0], axis=0)
        x = x - mean # the -= means can be read as x = x- np.mean(x)

        std = x.std(axis=0) # this calcualtes the standard deviation of the array
        std = np.repeat(std[np.newaxis,:, :], x.shape[0], axis=0)
        x = x / std # the /= means can be read as x = x/np.tsd(x)
        return x
...
def normalize_all_pixel(train_x, test_x):
    ...
    # test_x = ...
    def normalize(x):
        mean = x.mean() # calculates the mean of the array x
        x = x - mean # the -= means can be read as x = x- np.mean(x)

        std = x.std() # this calcualtes the standard deviation of the array
        x = x / std # the /= means can be read as x = x/np.tsd(x)
        return x
    train_x = normalize(train_x)
    test_x = normalize(test_x)
...
def add_one(x):
    ...
    """
    # [TODO 1.4]
    result = np.c_[ x, np.ones(x.shape[0]) ]
    return result
...
def test(y_hat, test_y):
    ...
    TP = np.sum(np.logical_and(y_hat >= 0.5, test_y == 1.0))
     
    #True Negative (TN): we predict a label of 0 (negative), and the true label is 0.
    TN = np.sum(np.logical_and(y_hat < 0.5, test_y == 0.0))
     
    #False Positive (FP): we predict a label of 1 (positive), but the true label is 0.
    FP = np.sum(np.logical_and(y_hat >= 0.5, test_y == 0.0))
     
    #False Negative (FN): we predict a label of 0 (negative), but the true label is 1.
    FN = np.sum(np.logical_and(y_hat < 0.5, test_y == 1.0))
